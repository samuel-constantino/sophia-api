...
import { reminderTasks, dailyResetTasks } from "../services/task";
...
export const dailyReset = async (req: Request, res: Response): Promise<any> => {
  try {
    const data = await dailyResetTasks();
    return res.status(200).json(data);
  } catch (error) {
    if (error instanceof Error) {
      return res.status(400).json({ errors: error.message });
